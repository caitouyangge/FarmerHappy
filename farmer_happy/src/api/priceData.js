...
export const priceDataService = {
  ...
  async downloadCsvFile(fileName) {
    try {
      logger.apiRequest('GET', `${API_URL}/price/download`, { fileName });
      logger.info('PRICE_DATA', '下载CSV文件', { fileName });
      ...
      logger.error('PRICE_DATA', '下载CSV文件失败', {
        errorMessage: error.response?.data?.message || error.message,
      }, error);
      throw new Error(error.response?.data?.message || error.message || '下载CSV文件失败，请稍后重试');
    }
  }
};
