...
public class OrderController {
    ...
    public Map<String, Object> applyRefund(String orderId, RefundRequestDTO request) {
        ...
        try {
            ...
            if (message.contains("不存在") || message.contains("未启用")) {
                ...
                response.put("code", 403);
                response.put("message", message);
            } else if (message.contains("不允许") || message.contains("已完成") || message.contains("正在退款") || message.contains("未发货")) {
                response.put("code", 409);
                response.put("message", message);
            } else {
                response.put("code", 500);
    ...
    public Map<String, Object> getFarmerOrderList(String farmerPhone, String status, String title) {
        ...
        try {
            // 调用服务层获取订单列表
            OrderListResponseDTO result = ((OrderServiceImpl) orderService).getFarmerOrderList(farmerPhone, status, title);
    ...
    public Map<String, Object> getFarmerOrderDetail(String orderId, String farmerPhone) {
        ...
        try {
            // 调用服务层获取订单详情
            OrderDetailResponseDTO result = ((OrderServiceImpl) orderService).getFarmerOrderDetail(orderId, farmerPhone);
